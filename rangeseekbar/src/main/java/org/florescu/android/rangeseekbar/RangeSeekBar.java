...
public class RangeSeekBar<T extends Number> extends ImageView {
			...
			case MotionEvent.ACTION_MOVE:
				if (pressedThumb != null) {
					...
					if (notifyWhileDragging && listener != null) {
						listener.onRangeSeekBarValuesChanged(this, pressedThumb, getSelectedMinValue(), getSelectedMaxValue());
					}
				}
				break;
	...
	protected synchronized void onDraw(@NonNull Canvas canvas) {
		...
		if (showTextAboveThumbs || activateOnDefaultValues ) {
			paint.setTextSize(textSize);
	...
	public interface OnRangeSeekBarChangeListener<T extends Number> {
		...
		void onRangeSeekBarValuesChanged(RangeSeekBar<T> bar, Thumb pressedThumb, T minValue, T maxValue);
	}
