...
public class UniversalVideoView extends SurfaceView
  ...
  private static final int VIDEO_PLAY = 0x001;
  private static final int VIDEO_STOP = 0x002;
  private Handler handler = new Handler() {
    @Override public void handleMessage(Message msg) {
      switch (msg.what) {
        case VIDEO_PLAY:
          if (mMediaController != null) mMediaController.setRootViewColor(Color.TRANSPARENT);
          break;
        case VIDEO_STOP:
          if (mMediaController != null) {
            mMediaController.setRootViewColor(getResources().getColor(android.R.color.black));
          }
          break;
      }
    }
...
//          mVideoHeight = 750;
  ...
  private boolean isFistComplete;
  ...
  private void startLoop() {
    while (isLooper) {
      if (mMediaPlayer != null && mMediaPlayer.getCurrentPosition() > 0) {
        handler.sendEmptyMessage(VIDEO_PLAY);
      } else {
  ...
  @Override public void setFullscreen(boolean fullscreen, int screenOrientation) {
