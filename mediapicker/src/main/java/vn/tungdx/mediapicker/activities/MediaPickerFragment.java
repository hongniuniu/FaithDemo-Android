	...
	public void onItemClick(AdapterView<?> parent, View view, int position,
		...
		if (object instanceof Cursor) {
			...
			if (mMediaType == MediaItem.PHOTO) {
				...
				path = MediaUtils.getRealImagePathFromURI(view.getContext().getContentResolver(), uri);
			} else {
				uri = MediaUtils.getVideoUri((Cursor) object);
				path = MediaUtils.getRealVideoPathFromURI(view.getContext().getContentResolver(), uri);
			}

			File file = new File(path);
			if (file == null || !file.exists() || file.isDirectory()) {
				Toast.makeText(mContext, "文件损坏，无法打开", Toast.LENGTH_SHORT).show();
				return;
			}
