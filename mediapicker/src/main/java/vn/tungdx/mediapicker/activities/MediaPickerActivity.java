 ...
 * {@link MediaPickerActivity#getMediaItemSelected(Context, Intent)} to get out media
...
public class MediaPickerActivity extends AppCompatActivity implements
	...
	private int checkValidVideo(Uri videoUri) {
		if (videoUri == null)
			return -2;
		long duration = MediaUtils.getDuration(this, videoUri);
	 ...
	 */
	public static ArrayList<MediaItem> getMediaItemSelected(Context context, Intent intent) {
		if (intent == null)
			return null;
		...
		for (MediaItem item : mediaItemList) {
			if (item.isVideo())
				item.setDuration(MediaUtils.getDuration(context, item.getUriOrigin()));
		}
